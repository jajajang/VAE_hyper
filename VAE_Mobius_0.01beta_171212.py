...
def train(epoch):
	...
	recon_batch= dec_(z)
        loss = loss_function(recon_batch, data, mu, logvar)+0.01*punisher(z,label)
        loss.backward()
...
def test(epoch):
	...
	test_loss += 0.01*punisher(z,label).data[0]
        if i == 0:
          n = min(data.size(0), 8)
          comparison = torch.cat([data[:n],
                                  recon_batch.view(args.batch_size, 1, 28, 28)[:n]])
          save_image(comparison.data.cpu(),
                     'results_mobi_001/reconstruction_' + str(epoch) + '.png', nrow=n)
...
for epoch in range(1, args.epochs + 1):
    ...
    save_image(sample.data.view(64, 1, 28, 28),'results_mobi_001/sample_' + str(epoch) + '.png')
