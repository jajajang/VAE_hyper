...
parser.add_argument('--no-cuda', action='store_true', default=Trues,
...
def train(epoch):
    ...
    for batch_idx, (data, label) in enumerate(train_loader):
        ...
        if (not go_skip):
            ...
            logvar_sum += torch.sum(logvar, dim=0)
            optimizer_enc.step()
            optimizer_dec.step()
        else:
            optimizer_enc.zero_grad()
    ...
    for i, (data, label) in enumerate(test_loader):
        ...
        if i == 0:
            ...
            save_image(comparison.data.cpu(),
                       'results_euc/reconstruction_d5_' + str(epoch) + '.png', nrow=n)
